...
non_mutual_following = Gauge('bluesky_non_mutual_following', 'People you follow who dont follow back')
followers_only = Gauge('bluesky_followers_only', 'People who follow you but you dont follow back')
api_requests = Counter('bluesky_api_requests_total', 'Total API requests', ['endpoint', 'status'])
collection_duration = Histogram('bluesky_collection_duration_seconds', 'Collection job duration')
...
def report():
    ...
    try:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('SELECT MAX(run_date) FROM collection_log WHERE status = "success"')
            last_run = cursor.fetchone()[0]
            if last_run:
                last_updated = datetime.fromisoformat(last_run).strftime('%Y-%m-%d %H:%M:%S')
            conn.close()
        except Exception:
            pass
        ...
        hidden_categories = {"muted": {"accounts": muted_accounts, "count": len(muted_accounts)}, "blocked": {"accounts": blocked_accounts, "count": len(blocked_accounts)}}
        ...
        import traceback
        error_html = '<h1>Error loading report</h1><p>' + str(e) + '</p><pre>' + traceback.format_exc() + '</pre>'
...
def api_follower_growth():
    ...
    try:
        ...
        api_requests.labels(endpoint='/api/graphs/follower-growth', status='success').inc()
        return jsonify(data)

    except Exception as e:
        logger.error(f"/api/graphs/follower-growth error: {e}")
        api_requests.labels(endpoint='/api/graphs/follower-growth', status='error').inc()
...
def api_engagement_timeline():
    ...
    try:
        ...
        api_requests.labels(endpoint='/api/graphs/engagement-timeline', status='error').inc()
        return jsonify({'error': str(e)}), 500
...
def api_posting_frequency():
    ...
    try:
        ...
        api_requests.labels(endpoint='/api/graphs/posting-frequency', status='success').inc()
        return jsonify(data)

    except Exception as e:
        logger.error(f"/api/graphs/posting-frequency error: {e}")
        api_requests.labels(endpoint='/api/graphs/posting-frequency', status='error').inc()
        return jsonify({'error': str(e)}), 500
...
def api_engagement_breakdown():
    ...
    try:
        ...
        api_requests.labels(endpoint='/api/graphs/engagement-breakdown', status='success').inc()
        return jsonify(data)

    except Exception as e:
        logger.error(f"/api/graphs/engagement-breakdown error: {e}")
        api_requests.labels(endpoint='/api/graphs/engagement-breakdown', status='error').inc()
        return jsonify({'error': str(e)}), 500
