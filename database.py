...
class Database:
        ...
        with self.get_connection() as conn:
            ...
            for follower in followers:
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (collection_date, follower['did'], follower['handle'],
                       follower.get('display_name', ''), follower.get('avatar_url', ''),
                       follower.get('bio', '')))
            ...
            for follow in following:
                cursor.execute('''
                    ...
                    (collection_date, did, handle, display_name, avatar_url, bio)
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (collection_date, follow['did'], follow['handle'],
                       follow.get('display_name', ''), follow.get('avatar_url', ''),
                       follow.get('bio', '')))
            
            logger.info(f"Saved snapshot: {len(followers)} followers, {len(following)} following")
            ...
            if muted is not None:
                for account in muted:
                    cursor.execute('''
                        ...
                        (collection_date, did, handle, display_name, avatar_url, bio)
                        VALUES (?, ?, ?, ?, ?, ?)
                    ''', (collection_date, account['did'], account['handle'],
                           account.get('display_name', ''), account.get('avatar_url', ''),
                           account.get('bio', '')))
            ...
            if profile_counts:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (collection_date, profile_counts['followers'], profile_counts['following'],
                       api_followers, api_following, hidden_followers, hidden_following,
                       muted_count, blocked_count, suspected))
                
                logger.info(f"Saved daily counts - Hidden: {hidden_followers} followers ({muted_count} muted, {blocked_count} blocked, {suspected} suspected blocks/suspensions)")
    ...
    def detect_changes(self, collection_date):
        ...
        with self.get_connection() as conn:
            ...
            # Get current counts
            cursor.execute('SELECT COUNT(*) FROM followers_snapshot WHERE collection_date = ?', (collection_date,))
            follower_count = cursor.fetchone()[0]
            
            cursor.execute('SELECT COUNT(*) FROM following_snapshot WHERE collection_date = ?', (collection_date,))
            following_count = cursor.fetchone()[0]
            ...
            cursor.execute('''
                ...
                (metric_date, follower_count, following_count, unfollower_count, new_follower_count)
                VALUES (?, ?, ?, ?, ?)
            ''', (collection_date, follower_count, following_count, unfollower_count, new_follower_count))
            
            logger.info(f"Changes detected: {unfollower_count} unfollowers, {new_follower_count} new followers")
    ...
    def get_stats(self, days=30):
        ...
        with self.get_connection() as conn:
            ...
            cutoff_date = (datetime.strptime(latest_date, '%Y-%m-%d').date() - timedelta(days=days)).isoformat()
            cursor.execute('''
    ...
    def get_unfollowers(self, days=30):
        ...
        with self.get_connection() as conn:
            ...
            ''', (cutoff_date,))
            return [{'handle': row[0], 'did': row[1], 'date': row[2]} for row in cursor.fetchall()]
    ...
    def log_collection(self, collection_date, status, followers_collected, following_collected, error_message, duration):
        """Log collection run"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (datetime.now(), status, followers_collected, following_collected, error_message, duration))
    ...
    def get_engagement_stats(self, days=30):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='post_engagement'")
    ...
    def get_hidden_follower_analytics(self, days=30):
        ...
        with self.get_connection() as conn:
            ...
            # Check if table exists
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='daily_counts'")
            if not cursor.fetchone():
                return None
            ...
            for row in cursor.fetchall():
                if row[1] is not None:  # Skip first row (no previous data)
                    ...
                    if row[1] < 0 and row[2] < 0:
                        ...
                        change_type = 'cleanup'  # Only profile decreased = index cleanup
    ...
    def get_advanced_metrics(self):
        ...
        with self.get_connection() as conn:
            ...
            return {
                ...
                'total_reposts': row[6],
                'total_replies': row[7],
                'engagement_rate': round((row[1] + row[2] * 2 + row[3]) if row[1] else 0, 2),
