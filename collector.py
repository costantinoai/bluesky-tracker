...
class BlueskyCollector:
    ...
    def fetch_engagement_data(self):
        ...
        try:
            ...
            logger.info(f"Collected engagement data for {len(engagement)} posts (including indirect)")
            return engagement
    ...
    def fetch_indirect_engagement(self, post_uri):
        ...
        try:
            ...
            if quote_count > 0:
                logger.info(f"Found {quote_count} quote posts with {total_likes} likes, {total_reposts} reposts, {total_replies} replies, {total_bookmarks} bookmarks")
    ...
    def fetch_profile_counts(self):
        ...
        try:
            ...
            logger.info(f"Profile counts: {profile_followers} followers, {profile_following} following")
            return {
    ...
    def fetch_interactions(self):
        ...
        try:
            ...
            logger.info(f"Collected interaction data for {len(interactions_list)} users")
    ...
    def collect(self):
        ...
        try:
            ...
            logger.info(f"Collected {len(followers)} followers (API) vs {profile_counts['followers']} (profile)")

            following = self.fetch_all_following()
            logger.info(f"Collected {len(following)} following (API) vs {profile_counts['following']} (profile)")
            ...
            self.db.log_collection(collection_date, 'success',
                                  len(followers), len(following), None, duration)
            ...
            self.db.log_collection(collection_date, 'failed', None, None,
                                  str(e), duration)
            return False